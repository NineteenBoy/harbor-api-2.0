...
class HarborClient(object):
    ...
    def login(self):
        login_data = requests.post('%s://%s/c/login' %
                                   ...
                                   data={'principal': self.user,
                                         'password': self.password}, cookies=self.cookies, headers=self.headers, verify=False)
        print(login_data)
    ...
    def logout(self):
        requests.get('%s://%s/c/logout' % (self.protocol, self.host),
                     cookies={'sid': self.session_id}, verify=False)
    ...
    def get_repository_artifacts(self, project_name, repository_name):
        ...
        while True:
            ...
            response = requests.get(path,
                                    cookies={'sid': self.session_id}, timeout=60, verify=False)
            if response.status_code == 200:
                logging.debug(
    ...
    def delete_repository(self, project_name, repository_name, tag=None):
        ...
        response = requests.delete(path,
                                   cookies=self.cookies_new, headers=self.headers, verify=False)
        if response.status_code == 200:
            result = True
    ...
    def del_repository_tag(self, project_name, repository_name, reference_hash, tag):
        ...
        response = requests.delete(path, cookies=self.cookies_new, headers=self.headers, verify=False)
        if response.status_code == 200:
            result = True
